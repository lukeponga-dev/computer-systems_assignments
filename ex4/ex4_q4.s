...
loop:
	lw $6, counter($0) 		# Load counters value into $6
	
	lw $7,   terminate($0)		# Check if termination flag is set
	beqz $7, exit			# If so, exit
	
	lw $3, print($0)		# Load print
	beqz $3, writessd		# If print not initialisation, keep incrementing counter

########################
# serial transmit data #
########################
return:
	lw $7, serial_status($0)	# Get the secound serial port status
	andi $7, $7, 0x2		# Check if the TDS bit is set
	beqz $7, return		# If not, loop and try again
...
newline:
	lw $7, serial_status($0)	# Get the secound serial port status
	andi $7, $7, 0x2		# Check if the TDS bit is set
	beqz $7, newline		# If not, loop and try again
...
first_char:
	lw $7, serial_status($0)	# Get the secound serial port status
	andi $7, $7, 0x2		# Check if the TDS bit is set
	beqz $7, first_char		# If not, loop and try again
...
secound_char:
	lw $7, serial_status($0)	# Get the secound serial port status
	andi $7, $7, 0x2		# Check if the TDS bit is set
	beqz $7, secound_char		# If not, loop and try again
...
dot:
	lw $7, serial_status($0)	# Get the secound serial port status
	andi $7, $7, 0x2		# Check if the TDS bit is set
	beqz $7, dot			# If not, loop and try again
...
third_char:
	lw $7, serial_status($0)	# Get the secound serial port status
	andi $7, $7, 0x2		# Check if the TDS bit is set
	beqz $7, third_char		# If not, loop and try again
...
fourth_char:
	lw $7, serial_status($0)	# Get the secound serial port status
	andi $7, $7, 0x2		# Check if the TDS bit is set
	beqz $7, fourth_char		# If not, loop and try again
	...
	sw $0, print($0)
writessd:	
	divui $6, $6, 100		# Divide timer by 100 to display elapsed 
# write counter to SSD
	remui $5, $6, 10		# lower right SSD
	sw $5, writessd_lr($0)		# Save value
	divui $6, $6, 10		# Go to next digit
		
	remui $5, $6, 10		# lower left SSD
...
# Reset the counter to 0
	...
	bnez $13, irq3_print	  	# If so, counter is incrementing
...
# serial #
.equ serial_td,	0x71000
.equ serial_rd,	0x71001
.equ serial_control,	0x71002
.equ serial_status,	0x71003
.equ serial_interrupt, 0x71004
