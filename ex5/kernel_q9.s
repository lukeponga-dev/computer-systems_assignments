...
## Setting up idle task ##
	la $1, IDLE_pcb                         # load the IDLE task
	la $2, IDLE_pcb                         # set IDLE to itself
	sw $2, pcb_link($1)
...
# Handle the timer interrupt
	...
	lw $13, time_slice($0)	        		# load timeSlice
	subi $13, $13, 1							# decrement by 1
	sw $13, time_slice($0)				# Save time_slice
	
	lw $13, time_slice($0)	        		# task Slice has finshed
...
timeSliceGame:
	lw $13, current_task($0)		# load current task
	sw $1, pcb_reg1($13)           	# save register 1 to use 
	la $1, games_pcb                 	# load games
	
	sequ $1, $13, $1					# check current task is games
	beqz $1, timeSliceOthers		# if not set time slice to 1 tick
	                     
	addui $13, $0, 4						#set timeslice to 4 ticks
	sw $13, time_slice($0)			#store it
	rfe		                                	#return from exception 

timeSliceOthers:
	lw $1, pcb_reg1($13)            # reload register 1
	addui $13, $0, 1                    #set timeslice to 1 tick
	sw $13, time_slice($0)           #store time
	rfe				#return from exception
		
exit_process:
# Manipulate linked list
	    lw $1, current_task($0)  	# Load process that is exiting
	    lw $2, pcb_link($1) 		# Load process that ran before this one
	    
	    sequ $3, $1, $2     	# Check if this is the last task
	    bnez $3, IDLEJ     	 	# exit last task
...
IDLEJ:
	...
	la	$2, IDLE_pcb		#load IDLE task
	sw	$2, pcb_link($1)	#change current link to IDLE
	j 	schedule		#re load task
...
IDLE_main:
	...
	addui	$1, $0, 0x40	
	sw	$1, SSD_LR($0)
	addui	$1, $0, 0x40	#loop print '-. -' sleepy face
	sw	$1, SSD_LL($0)
	addui	$1, $0, 0x40		
	sw	$1, SSD_UR($0)
	addui	$1, $0, 0x40
