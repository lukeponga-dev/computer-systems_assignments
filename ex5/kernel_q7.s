...
## Interrupt handler ##
	movsg $1, $evec 			# Copy the old handler's address to $2
	sw $1, old_vector($0)	# Save it to memory
	la $1, handler 					# Get the addres of handler
	movgs $evec, $1			# Copy the new address of the handler into $evec
	
## Timer Instructions ##
	sw $0, timerInterrupt_reg($0)	        # Acknowledge any outstanding interrupts
	addui $1, $0, 24									# put out count value into the timer load reg
	sw $1, timerLoad_reg($0)					# save it to memory
	addui $1, $0, 0x3								# Enable the timer and set auto-restard mode
	sw $1, timerControl_reg($0)				# Save it to the timer control register
